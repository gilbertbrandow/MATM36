...
Let $B = \{a_{i_1}, \cdots,  a_{i_m}\} \subseteq A$. Since $B$, subset $m \leq n$. Define $\mathbf{x}$ such that $x_j = \mathbf{1}_{\{j \in \{i_1, \cdot, i_m\}\}}$. 
Then $f(\mathbf{x}) = B$, since $B$ arbitrary $f$ surjective and therefore bijective.
\\
\\
\textbf{b: } Since for any set $A$, there exists a bijective $f: \{0, 1\}^n \to \mathcal{P(A)}$. 
Consider 
\[
g: \{0, 1\}^n \to \{1, \cdots, 2^n\}, \ (x_1, \cdots, x_n) \mapsto \left(\sum_{k = 0}^{n}2^kx_{k-1}\right) - 1 
\] 
$g$ is clearly bijective.
Let $h := f \circ g: \mathcal{P(A)} \to \{1, \cdots, 2^n\}$, since compositions preserves bijections, $h$ is a bijection and therefore card $\mathcal{P(A)} = 2^{\text{card}A}$
\end{document}