...
\textbf{(d)}
\begin{proof}
Let $W\in\mathcal T'$. Then $W=\bigcup_{\alpha}(U_\alpha\times V_\alpha)$ with each $U_\alpha$ open in $X$ and $V_\alpha$ open in $Y$.
Fix $\alpha$ and $(x,y)\in U_\alpha\times V_\alpha$. By (c)(i) there exists $\varepsilon>0$ such that
\[
B^d_\varepsilon((x,y))\subseteq U_\alpha\times V_\alpha.
\]
Hence each $U_\alpha\times V_\alpha$ is $d$-open, so $U_\alpha\times V_\alpha\in\mathcal T$, and therefore $W\in\mathcal T$ (arbitrary unions of $d$-open sets are $d$-open).

Let $W\in\mathcal T$ and fix $(x,y)\in W$. Then there exists $\varepsilon>0$ such that
\[
B^d_\varepsilon((x,y))\subseteq W.
\]
By (c)(ii) there exist $U$ open in $X$ and $V$ open in $Y$ with
\[
(x,y)\in U\times V \subseteq B^d_\varepsilon((x,y))\subseteq W.
\]
Thus every point of $W$ lies in some basis element $U\times V\in\mathcal B$ contained in $W$, so $W$ is a union of such basis elements and hence $W\in\mathcal T' \implies \mathcal T \subseteq \mathcal T'$. Therefore $\mathcal T=\mathcal T'$.
\end{proof}

\textbf{(e)}
\begin{proof}
    Assume $X=Y=\mathbb R$ and $d_X=d_Y=|\cdot|$. Then the metric $d$ on $\mathbb R^2$ from 2(a) is
\[
d((x_1,y_1),(x_2,y_2))=\max\{|x_1-x_2|,\ |y_1-y_2|\}=d_\infty\big((x_1,y_1),(x_2,y_2)\big).
\]
By (d) the metric topology $\mathcal T=\mathcal T(d_\infty)$ equals the topology $\mathcal T'$ generated by the basis
$\{U\times V:\ U,V\text{ open in }\mathbb R\}$. The metrics $d_2$ and $d_\infty$ on $\mathbb R^2$ are uniformly equivalent, hence they induce the same topology.
In conclusion $\mathcal T(d_2) = \mathcal T(d_{\infty}) = \mathcal T = \mathcal T'$
\end{proof}
